...
for cond_num=1:length(cond_names)
    ...
    %won't see a difference in ratings between the levels.
        if points_counter < 1 || thresh_stim(1)>=4.6 || thresh_stim(2)>=4.6
            title({'';sprintf('\n %s, r^2 = %.2f \n L=%s \n',...
                cond_names{cond_num}, p_r2,print_thresh_values(thresh_stim))},'Color',[0.6350 0.0780 0.1840]);
...
results_folder = append(current_folder,'\unreal_04\sub_',sub_name,'\results_sub_',sub_name,'_attempt_',num2str(attempt)); %cd to the current subjects' results folder
